...
export class RatesRegistryService {
  private readonly ratesServices: Map<string, AbstractRateService<unknown, unknown>> =
    new Map();

  /**
   * Register a rates service for an integration
   */
  registerService(service: AbstractRateService<unknown, unknown>): void {
   ...
   */
  getService(name: string): AbstractRateService<unknown, unknown> | undefined {
